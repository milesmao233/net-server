...
const updateUserPassword = (request) => {
    const form = request.form()
    form.password = User.saltedPassword(form.password)
    User.update(form)
}

const editPassword = (request) => {
    const userId = Number(request.query.id)
    const u = User.findBy('id', userId)
    const body = template('admin_password_edit.html', {
        user: u,
    })
    return htmlResponse(body)
}
...
const routeMapper = () => {
    const d = {
        ...
        '/admin/password/update': updateUserPassword,
        '/admin/password/edit': editPassword,
    }
    return d
}
